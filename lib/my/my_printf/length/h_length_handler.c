...
static char *h_unsigned_handler(char specifier, va_list args)
{
...
char *h_length_handler(spec_t *spec, va_list args, size_t size)
{
    ...
    if (is_in_char(spec->specifier, "di"))
        return safe_return(signed_to_str((short)va_arg(args, int)));
    str = h_unsigned_handler(spec->specifier, args);
    if (str)
        return str;
